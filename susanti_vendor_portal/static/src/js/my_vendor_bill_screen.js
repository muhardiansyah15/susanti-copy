odoo.define('susanti_vendor_portal.my_vendor_bill_screen', function(require) {
    "use strict";
    var PublicWidget = require('web.public.widget');
    var ajax = require('web.ajax');
    var Template = PublicWidget.Widget.extend({
        selector: '.my_vendor_bill_screen',
        events: {
        ...
        _onClickUpdateAttachment: function(ev) {
            ...
            if (self.fileList) {
                ajax.jsonRpc('/payment_proof/submit', 'call', {
                   'invoice_id': Number(self.$(ev.currentTarget).attr('value')),
        ...
        _onClickShowReceipt: function() {
            ...
            ajax.jsonRpc('/my_vendor_bill_screen/show_updated', 'call', {
                ...
                if (attachment_ids.length > 0) {
                    ...
                    self.$el.find("#showing_updated_receipt").empty();
                    self.$el.find('#showing_updated_receipt').append("<p style='color: red;'>There is no attachments for this bill.</p>")
                }
            })
        }
    })
    PublicWidget.registry.my_vendor_bill_screen = Template;
    return Template;
})